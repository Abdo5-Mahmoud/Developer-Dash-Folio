...
import type { ProjectInput, ProjectStatus, Skill, Technology } from "@/lib/types";
import type {
  AIPromptEntry,
  AIMistakeEntry,
  AlgorithmEntry,
...
export function ProjectForm({
  ...
  const [savingStatus, setSavingStatus] = React.useState<ProjectStatus | null>(null);
  ...
  function toggleTechnology(technology: Technology) {
    const exists = values.techStack.some((item) => item.technologyId === technology.id);
    const next = exists
      ? values.techStack.filter((item) => item.technologyId !== technology.id)
      : [...values.techStack, { technologyId: technology.id, name: technology.name }];
    set("techStack", next);
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      {requiredMissing && (
        <Callout type="warning" title="Before you publish">
          Title, summary, full description, and at least one technology are required.
        </Callout>
      )}
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="flex flex-col gap-5">
          ...
          <Field label="Slug" hint="Optional on create. Existing project slugs stay stable.">
          ...
          <Field label="Summary" hint="Shown on cards and the listing page - 1 or 2 sentences.">
        ...
        <TabsContent value="media" className="flex flex-col gap-5">
          <Field label="Cover image URL" hint="Used by cards and the project header.">
            <Input value={values.coverImage ?? ""} onChange={(e) => set("coverImage", e.target.value)} />
          </Field>
          <Field label="Cover image alt text">
            <Input value={values.coverImageAlt ?? ""} onChange={(e) => set("coverImageAlt", e.target.value)} />
          </Field>
          <Field label="Gallery" hint="Additional screenshots or diagrams.">
            <div className="flex flex-col gap-3">
              {values.gallery.map((item, index) => (
                <div key={index} className="grid gap-3 rounded-lg border border-border bg-surface p-4 md:grid-cols-[1fr_1fr_auto]">
                  <Input
                  ...
                  <Button
                    ...
                    onClick={() => set("gallery", values.gallery.filter((_, i) => i !== index))}
                  >
              ...
              <Button
                ...
                size="sm"
                className="w-fit"
                onClick={() => set("gallery", [...values.gallery, { url: "", alt: "" }])}
              >
        ...
        <TabsContent value="tech" className="flex flex-col gap-6">
          <Field label="Tech stack" hint="Required before publishing.">
            <ChoiceGrid
              items={technologies}
              isSelected={(item) => values.techStack.some((selected) => selected.technologyId === item.id)}
            ...
            <RepeatableField<AlgorithmEntry>
              ...
              renderItem={(item, update) => (
                <div className="flex flex-col gap-2">
                  ...
                  <Input placeholder="Complexity" value={item.complexity ?? ""} onChange={(e) => update({ complexity: e.target.value })} />
            ...
            <RepeatableField<PerformanceEntry>
              ...
              renderItem={(item, update) => (
                <div className="flex flex-col gap-2">
                  ...
                  <Input placeholder="Measured or estimated impact" value={item.impact ?? ""} onChange={(e) => update({ impact: e.target.value })} />
                </div>
              )}
            />
          </Field>
        ...
        <TabsContent value="ai" className="flex flex-col gap-6">
          <Callout type="info">Be specific. Paste real prompts when possible.</Callout>
          ...
          <Field label="AI prompts used">
            <RepeatableField<AIPromptEntry>
              ...
              renderItem={(item, update) => (
                <div className="flex flex-col gap-2">
                  <Input placeholder="Purpose or context" value={item.purpose} onChange={(e) => update({ purpose: e.target.value })} />
              ...
              renderItem={(item, update) => (
                <div className="flex flex-col gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => update({ alternatives: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })}
                  />
