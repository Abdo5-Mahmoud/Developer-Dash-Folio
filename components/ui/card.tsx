...
export function Card({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "rounded-lg border border-border bg-surface shadow-[var(--shadow-token-sm)] " +
          "transition-shadow duration-150 hover:shadow-[var(--shadow-token-md)]",
        className
      )}
      {...props}
    />
...
export function CardContent({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn("p-5 pt-0 text-sm text-foreground", className)} {...props} />;
}
